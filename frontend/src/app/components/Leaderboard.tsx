    ...
    <div className="min-h-screen bg-background pb-20">
      <div className="bg-gradient-to-br from-[#6C3FE8] to-[#8B5CF6] text-white p-6 rounded-b-3xl shadow-lg">
        <div className="text-center mb-5">
          ...
          <h1 className="text-3xl" style={{ fontWeight: 800 }}>Таблица лидеров</h1>
        </div>
        ...
        <div className="flex gap-2 bg-white/10 rounded-2xl p-1">
          ...
          >
              {key === "class" ? "Класс" : key === "school" ? "Школа" : "Город"}
            </button>
          ))}
        </div>
      </div>
          ...
          <>
            {topThree.length > 0 && (
              <div className="bg-white rounded-3xl p-5 shadow-sm">
                <div className="text-center text-sm text-muted-foreground mb-3">Top 3</div>
                    ...
                    <div key={entry.user.id} className="flex items-center justify-between bg-muted/50 rounded-xl p-3">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          ...
                          <div className="text-xs text-muted-foreground">{entry.user.school?.name ?? "School"}</div>
                        </div>
                      </div>
            ...
            <div className="bg-white rounded-3xl p-5 shadow-sm">
              <h2 className="text-lg mb-3" style={{ fontWeight: 700 }}>Все участники</h2>
              <div className="space-y-2">
                {items.map((entry) => (
                  <div
                    key={entry.user.id}
                  ...
                  >
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <div className="text-xs text-muted-foreground">{entry.user.city?.name ?? "City"}</div>
                      </div>
                    </div>
                    <div style={{ fontWeight: 700 }}>{entry.xp} XP</div>
                  </div>
              ...
              {currentUser && !items.some((item) => item.user.id === currentUser.user.id) && (
                <div className="mt-4 p-3 rounded-xl border border-primary bg-primary/5 text-sm">
                  Ваше место: <span style={{ fontWeight: 700 }}>#{currentUser.rank}</span>, XP: <span style={{ fontWeight: 700 }}>{currentUser.xp}</span>
                </div>
              )}
            </div>
          </>
